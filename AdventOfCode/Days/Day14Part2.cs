    ...
    public class Day14Part2
    {
        ...
        private class Map
        {
            ...
            public (Position nextPosition, bool Moved, bool Done) Fall(Position startPosition)
            {
                ...
                while ((nextPosition = MoveOne(lastPosition)) != null)
                ...
                bool done = restingPlace.Y == 0 && restingPlace.X == 500;
                if (!done)
                    takenSpots.Add(restingPlace);
                return (restingPlace, startPosition.Equals(restingPlace), done);
            }

            private Position? MoveOne(Position startPosition) => !IsTaken(startPosition.Down) ? startPosition.Down :
                    !IsTaken(startPosition.DownLeft) ? startPosition.DownLeft :
                    !IsTaken(startPosition.DownRight) ? startPosition.DownRight :
                    null;

            private bool IsTaken(Position p) => p.Y == maxY + 2 || takenSpots.Contains(p);
        }
